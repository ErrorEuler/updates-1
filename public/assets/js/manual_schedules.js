...
// Real-time validation throttle
let validationTimeout;
const DEBOUNCE_DELAY = 300;

// New: Real-time field validator
// Replace the existing validateFieldRealTime function
  ...
  validationTimeout = setTimeout(() => {
    ...
    fetch("/chair/generate-schedules", {
      ...
      .then(result => {
        ...
        resetConflictStyles(); // Clear previous warnings
...
function showNotification(message, type = "success", duration = 5000) {
  ...
  notificationDiv.className = `fixed top-4 right-4 z-50 max-w-sm w-full ${type === "success"
      ...
      : type === "error"
        ...
        : "fa-exclamation-triangle text-yellow-400"
            ...
            <div class="ml-3 flex-1">
                <p class="text-sm font-medium ${type === "success"
      ? "text-green-800"
      : type === "error"
        ? "text-red-800"
        : "text-yellow-800"
            ...
            <div class="ml-auto pl-3">
                <button class="inline-flex ${type === "success"
...
// Update handleScheduleSubmit to enhance conflict display
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (sectionSelect) {
    sectionSelect.addEventListener("change", (e) => {
      ...
      handleSectionChange(); // Keep existing
    });
  }
  const daySelect = document.getElementById("day-select");
  if (daySelect) {
    daySelect.addEventListener("change", (e) => {
      updateDayField(); // Existing
  ...
  // For course code (existing sync + validate if needed)
  const courseCodeInput = document.getElementById("course-code");
  if (courseCodeInput) {
    courseCodeInput.addEventListener("input", (e) => {
      syncCourseName(); // Existing
      // Optional: Validate course exists (already in autoFill)
    });
  }
  console.log("Real-time validation hooked up—warnings incoming!");
});
...
function filterSectionsByYearLevel() {
  ...
  if (foundMatchingSections) {
    resetConflictStyles(); // Clear if good
  } else {
  ...
  if (selectedOption && selectedOption.value) {
    ...
    detailsDiv.innerHTML = `
            <div class="flex justify-between items-center">
                ...
                <span class="text-blue-600">${yearLevel || "Unknown Year"
      }</span>
            </div>
