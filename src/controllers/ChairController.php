    ...
    public function generateSchedulesAjax()
    {
        ...
        switch ($action) {
            ...
            case 'validate_partial':
                ...
                if (($sectionId || $facultyId || $roomId) && $startTime && $endTime && $dayOfWeek) {
                    foreach ($daysToCheck as $day) {
                    ...
                    $conflicts[] = "Please enter start and end times to check for conflicts.";
                } elseif (!$dayOfWeek) {
                    $conflicts[] = "Please select a day to check for conflicts.";
                }

                echo json_encode([
                    'success' => true,
                    'conflicts' => array_unique($conflicts),
                    'message' => empty($conflicts) ? 'No conflicts detected' : implode('; ', $conflicts)
                ]);
                exit;
                break;
