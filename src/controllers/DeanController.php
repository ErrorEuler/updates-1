...
public function getPendingFacultyCount() {
    $userId = $_SESSION['user_id'] ?? null;
    if (!$userId) {
        error_log("getPendingFacultyCount: No user_id in session");
        return 0;
    }

    // Get dean's college_id (reuse logic)
    $query = "
        SELECT d.college_id
        FROM deans d
        WHERE d.user_id = :user_id AND d.is_current = 1
    ...
    $college = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$college) {
        error_log("getPendingFacultyCount: No college found for dean user_id: $userId");
        return 0;
    }
    $collegeId = $college['college_id'];

    // Count pending faculty (inactive, role 6)
    $query = "
        SELECT COUNT(*) 
        FROM users u
        JOIN faculty f ON u.user_id = f.user_id
        WHERE u.college_id = :college_id AND u.is_active = 0 AND u.role_id = 6
    ";
    $stmt = $this->db->prepare($query);
    $stmt->execute([':college_id' => $collegeId]);
    $count = $stmt->fetchColumn() ?: 0;

    error_log("getPendingFacultyCount: Fetched $count pending faculty for college_id=$collegeId");

    return (int) $count;
}

/**
 * Get shared statistics for dean layout (faculty, classrooms, departments, pending approvals).
 * @return array Stats array or error if no college found.
 */
public function getStats() {
    $userId = $_SESSION['user_id'] ?? null;
    if (!$userId) {
        error_log("getStats: No user_id in session");
        return ['error' => 'No session'];
    }

    // Get dean's college_id (reuse logic from dashboard)
    $query = "
        SELECT d.college_id, c.college_name
        FROM deans d
        JOIN colleges c ON d.college_id = c.college_id
    ...
    $college = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$college) {
        error_log("getStats: No college found for dean user_id: $userId");
        return ['error' => 'No college assigned'];
    }
    $collegeId = $college['college_id'];

    // Compute stats (reuse existing private methods)
    $stats = [
        'total_faculty' => $this->getCollegeStats($collegeId, 'faculty'),
        'total_classrooms' => $this->getCollegeStats($collegeId, 'classrooms'),
        'total_departments' => $this->getCollegeStats($collegeId, 'departments'),
        'pending_approvals' => $this->getPendingApprovals($collegeId)
    ];

    // Add total_pending for the schedule badge (non-approved schedules)
    $currentSemesterStmt = $this->db->prepare("SELECT semester_id FROM semesters WHERE is_current = 1 LIMIT 1");
    $currentSemesterStmt->execute();
    $currentSemesterId = $currentSemesterStmt->fetchColumn();
    if ($currentSemesterId) {
        $pendingStmt = $this->db->prepare("
            ...
            FROM schedules s
            JOIN departments d ON s.department_id = d.department_id
            WHERE d.college_id = :college_id AND s.semester_id = :semester_id AND s.status != 'Approved'
        ");
        $pendingStmt->execute([':college_id' => $collegeId, ':semester_id' => $currentSemesterId]);
        $stats['total_pending'] = (int) ($pendingStmt->fetchColumn() ?: 0);
    } else {
        $stats['total_pending'] = 0;
    }

    error_log("getStats: Fetched for college_id=$collegeId - Faculty: {$stats['total_faculty']}, Classrooms: {$stats['total_classrooms']}, Depts: {$stats['total_departments']}, Pending: {$stats['pending_approvals']}, Total Pending Schedules: {$stats['total_pending']}");
