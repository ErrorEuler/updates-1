    ...
    public function register()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $data = [
                ...
                // 🆕 FIX: Properly capture dual role data
                'has_dual_role' => isset($_POST['has_dual_role']) && $_POST['has_dual_role'] == '1' ? 1 : 0,
                'secondary_role_id' => !empty($_POST['secondary_role_id']) ? intval($_POST['secondary_role_id']) : null
            ];
            ...
            // 🆕 Validate dual role combination
            if ($data['has_dual_role'] && $data['secondary_role_id']) {
                $valid_combinations = [[4, 5], [5, 4]]; // Dean+Chair or Chair+Dean
                $is_valid = false;
                foreach ($valid_combinations as $combo) {
                    if ($data['role_id'] == $combo[0] && $data['secondary_role_id'] == $combo[1]) {
                        $is_valid = true;
                        break;
                    }
                }
                if (!$is_valid) {
                    $errors[] = "Invalid dual role combination. Only Dean/Program Chair combinations are allowed.";
                }
            }
