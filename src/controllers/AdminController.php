...
class AdminController
{
    ...
    public function getUsersTableData()
    {
        ...
        ");
            ...
            if (!$vpaa) {
                ...
                echo json_encode(['success' => false, 'error' => 'VPAA profile not found']);
                exit;
            }
            ...
            if ($tab !== 'all') {
                $users = array_filter($users, function ($user) use ($tab) {
                    return ($tab === 'active' && $user['is_active']) ||
                        ($tab === 'inactive' && !$user['is_active']) ||
                        ($tab === 'pending' && !$user['is_active']);
                });
            }
            ...
            error_log("getUsersTableData error: " . $e->getMessage());
            header('Content-Type: application/json');
    ...
    ");
        ...
        try {
            ...
            } elseif ($action === 'activate' || $action === 'approve') {
                $query = "UPDATE users SET is_active = 1 WHERE user_id = :user_id";
                $stmt = $this->db->prepare($query);
                $stmt->execute([':user_id' => $userId]);
